...
public class JSONToArrayListDecoder<M> implements IStringTranslator<ArrayList<M>>{
    ...
    public JSONToArrayListDecoder(IJSONArrayListDecoder<M> p_decoder){
        this.m_decoder = p_decoder;
    }


    @Override
    public ArrayList<M> decode(String p_input) throws Throwable{
        return this.m_decoder.decode((JSONArray) new JSONTokener(p_input).nextValue());
    }
}
